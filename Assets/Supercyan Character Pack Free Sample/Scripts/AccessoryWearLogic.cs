...
namespace Supercyan.FreeSample
{
    public class AccessoryWearLogic : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            foreach (var a in m_accessoriesToAttach) Attach(a);
        }
        ...
        public void Attach(AccessoryLogic accessory)
        {
            ...
            var newBones = GetBones(accessory.Renderer.bones, m_characterBones);
            if (newBones == null)
            {
        ...
        {
            var newBones = new Transform[accessoryBones.Length];

            for (var i = 0; i < accessoryBones.Length; i++)
            {
                var bone = FindBone(m_characterRenderer.rootBone, accessoryBones[i].name);
